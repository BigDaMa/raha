...
class DetectionParallel(Detection):
    ...
    def run_outlier_strategy(self, configuration, dataset_ref, strategy_name_hash):
        ...
        if os.path.exists(dboost_result_path) and os.path.getsize(dboost_result_path) > 0:
            dboost_dataframe = pandas.read_csv(dboost_result_path, sep=",", header=None, encoding="utf-8",
                                               dtype=str, keep_default_na=False, low_memory=False).apply(
    ...
    def parallel_strat_runner_process(self, args):
        ...
        if self.SAVE_RESULTS:
            dataset = dp.DatasetParallel.load_shared_dataset(dataset_ref)
            pickle.dump(strategy_results, open(
                os.path.join(dataset.results_folder, "strategy-profiling", strategy_name_hashed + ".dictionary"), "wb"))
        if self.VERBOSE:
    ...
    def setup_knowledge_violation_metadata(dataset_ref):
        ...
        """
        configurations = []
        paths = [os.path.join(os.path.dirname(__file__), "tools", "KATARA", "knowledge-base", path) for path in
    ...
    def run_strategies(self, dataset):
        ...
        if self.STRATEGY_FILTERING:
            ...
            return raha.utilities.get_selected_strategies_via_historical_data(dataset.dictionary,
                                                                              self.HISTORICAL_DATASETS)
    ...
    def generate_features_one_col(self, dataset_ref, column_index):
        ...
        if self.TFID_ENABLED:
            ...
            try:
                ...
                feature_vectors = numpy.column_stack((feature_vectors, numpy.array(tfid_features.todense())))
            except:
                pass
    ...
    def generate_features(self, dataset):
        ...
        futures.append(client.map(self.generate_features_one_col, [dataset.own_mem_ref] * dataset.dataframe_num_cols,
    ...
    def sample_tuple(self, dataset):
        ...
        for j in numpy.arange(dataset.dataframe_num_cols):
            for c in dataset.clustering_results[j][1][k]:
                dataset.labels_per_cluster[(j, c)] = {cell: dataset.labeled_cells[cell][0] for cell in
                                                      dataset.clustering_results[j][1][k][c]
        ...
        if self.CLUSTERING_BASED_SAMPLING:
            ...
            for i in numpy.arange(dataset.dataframe_num_rows):
                if i not in dataset.labeled_tuples:
                    score = 0.0
                    for j in numpy.arange(dataset.dataframe_num_cols):
                        if dataset.clustering_results[j][1][k]:
                            cell = (i, j)
                            c = dataset.clustering_results[j][2][k][cell]
    ...
    def propagate_labels(self, dataset):
        """
        ...
        for j in numpy.arange(dataset.dataframe_num_cols):
            cell = (dataset.sampled_tuple, j)
            if cell in dataset.clustering_results[j][2][k]:
                c = dataset.clustering_results[j][2][k][cell]
                    ...
                    if len(dataset.labels_per_cluster[(j, c)]) > 0:
                        match self.LABEL_PROPAGATION_METHOD:
                            case constants.HOMOGENEITY:
                                ...
                                if sum(dataset.labels_per_cluster[(j, c)].values()) in [0, len(
                                        dataset.labels_per_cluster[(j, c)])]:
                                    for cell in dataset.clustering_results[j][1][k][c]:
                                        dataset.extended_labeled_cells[cell] = cluster_label
                            case constants.MAJORITY:
                                cluster_label = round(sum(dataset.labels_per_cluster[(j, c)].values()) / len(
                                    dataset.labels_per_cluster[(j, c)]))
                                for cell in dataset.clustering_results[j][1][k][c]:
                                    dataset.extended_labeled_cells[cell] = cluster_label
                            case _:
                                raise ValueError("The Label Propagation Method" + str(
                                    self.LABEL_PROPAGATION_METHOD) + " is not supported! Try homogeneity or majority.")
        if self.VERBOSE:
            print("The number of labeled data cells increased from {} to {}.".format(len(dataset.labeled_cells),
                                                                                     len(dataset.extended_labeled_cells)))
    ...
    def predict_labels_single_col(classification_model_name, verbose, dataset_ref, column_index):
        ...
        x_train = [feature_vectors[i, :] for i in numpy.arange(dataset.dataframe_num_rows) if
                   (i, column_index) in extended_labeled_cells]
        y_train = [extended_labeled_cells[(i, column_index)] for i in numpy.arange(dataset.dataframe_num_rows) if
                   (i, column_index) in extended_labeled_cells]
        ...
        # Check if all cells are 0's
            ...
            match classification_model_name:
                ...
                case _:
                    raise ValueError(
                        "Classification Model Name " + str(classification_model_name) + " is not supported!")
        ...
        for i, predicted_label in enumerate(predicted_labels):
            if (i in labeled_tuples and extended_labeled_cells[(i, column_index)]) or (
                    i not in labeled_tuples and predicted_label):
    ...
    def predict_labels(self, dataset):
        ...
        futures.append(client.map(
            ...
            [self.CLASSIFICATION_MODEL] * column_count, [self.VERBOSE] * column_count,
            [dataset.own_mem_ref] * column_count, numpy.arange(column_count))
        )
        results = client.gather(futures=futures, direct=True)[0]
    ...
    def run(self, dataset_dictionary):
        ...
        self.generate_features(dataset_par)
