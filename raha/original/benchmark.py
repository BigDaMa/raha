...
class Benchmark:
    ...
    def experiment_1(self):
        ...
        for r in range(self.RUN_COUNT):
            detector = raha.sequential.detection.Detection()
            ...
            for dataset_name in self.DATASETS:
                ...
                for tuple_wise_system in tuple_wise_systems:
                    ...
                    if tuple_wise_system == "Raha":
                        detection_dictionary = detector.run(dataset_dictionary)
                    er = raha.sequential.utilities.get_tuple_wise_evaluation(d, detection_dictionary)
    ...
    def experiment_4(self):
        ...
        for dataset_name in historical_datasets:
            ...
            raha.sequential.utilities.dataset_profiler(dataset_dictionary)
            raha.sequential.utilities.evaluation_profiler(dataset_dictionary)
            ...
            for dataset_name in self.DATASETS:
                ...
                for strategy_filtering_approach in strategy_filtering_approaches:
                    if strategy_filtering_approach == "No Strategy Filtering":
                        ...
                        detection_dictionary = detector.run(dataset_dictionary)
                        strategies_count, runtime = raha.sequential.utilities.get_strategies_count_and_runtime(dataset_dictionary)
                    elif strategy_filtering_approach == "Strategy Filtering via Historical Data":
                        selected_strategies = raha.sequential.utilities.get_selected_strategies_via_historical_data(dataset_dictionary, historical_dataset_dictionaries)
                        detection_dictionary = raha.sequential.utilities.error_detection_with_selected_strategies(dataset_dictionary, selected_strategies)
                        ...
                        worst_strategies, random_strategies, best_strategies = raha.sequential.utilities.get_selected_strategies_via_ground_truth(dataset_dictionary, filtered_strategies_count)
                        if strategy_filtering_approach == "Strategy Filtering via Least Effective Selection":
                            detection_dictionary = raha.sequential.utilities.error_detection_with_selected_strategies(dataset_dictionary, worst_strategies)
                        ...
                        if strategy_filtering_approach == "Strategy Filtering via Uniform Selection":
                            detection_dictionary = raha.sequential.utilities.error_detection_with_selected_strategies(dataset_dictionary, random_strategies)
                        ...
                        if strategy_filtering_approach == "Strategy Filtering via Most Effective Selection":
                            detection_dictionary = raha.sequential.utilities.error_detection_with_selected_strategies(dataset_dictionary, best_strategies)
    ...
    def experiment_6(self):
        ...
        d_tax = raha.sequential.dataset.Dataset(dataset_dictionary)
        for r in range(self.RUN_COUNT):
            detector = raha.sequential.detection.Detection()
            detector.VERBOSE = False
